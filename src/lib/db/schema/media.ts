...
export const media = sqliteTable(
	'media',
	{
		...
		// NOT unique on its own: the same Shopify MediaImage can be attached to
		// multiple products, so identity is (entity_type, entity_id, shopify_id).
	...
	(table) => [
		// A Shopify media gid is unique per entity, not globally — lets a shared
		// image attach to multiple products while staying idempotent on re-import.
		uniqueIndex('media_entity_shopify_idx').on(
			table.entityType,
			table.entityId,
			table.shopifyId
		)
