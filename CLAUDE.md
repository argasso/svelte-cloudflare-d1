...
- **Rich text**: author `description` and page `content` hold Shopify rich-text **JSON strings** (a small node set: paragraph, heading, list, link, text with bold/italic). The storefront renders them via `convertSchemaToHtml` in `src/lib/utils/richtext.ts`. Admin editing uses a TipTap WYSIWYG editor (`src/lib/components/RichTextEditor.svelte`) over the bidirectional converter in `src/lib/utils/tiptap.ts` (`shopifyToTiptap`/`tiptapToShopify` — note links are *nodes* in Shopify but *marks* in TipTap, and list-items need a paragraph wrapper). Remote form schemas validate/normalize the submitted JSON with `richTextField` (empty → null). The editor is configured to only produce nodes Shopify can store; don't add toolbar features beyond that set or they'll drop on save.
